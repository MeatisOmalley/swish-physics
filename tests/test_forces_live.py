...
from swish_physics.data import curves, serialize
...
def build(with_thigh=False):
    ...
    bpy.ops.swish.group_new()
    group = rig.swish.groups[0]
...
rig, group = build()
bpy.context.view_layer.objects.active = rig
check("Add Force makes a Basic force", bpy.ops.swish.force_add(kind="BASIC") == {"FINISHED"}
...
rig, group = build()
bpy.context.view_layer.objects.active = rig
bpy.ops.swish.force_add(kind="WIND")
...
check("Add Sync Bone follows the active bone and targets the selected chain bones",
      bpy.ops.swish.sync_add() == {"FINISHED"} and group.sync_bones[0].bone == "thigh"
...
rig, group = build()
bpy.context.view_layer.objects.active = rig
bpy.ops.swish.force_add(kind="CURVE")
bpy.ops.swish.force_add(kind="GRAVITY")
