...
6. **Keyed chain channels are the input.** In a post-frame-change handler, after Blender has evaluated animation, a keyed chain channel holds its animated value: that is the solver's pose input. Unkeyed chain bones use their rest pose. Which channels are keyed is read from the action when simulation starts and whenever animation changes. We never evaluate fcurves ourselves.
...
10. **Colliders are real objects.** A mesh object parented to a bone, its shape generated by one shared Geometry Nodes group from modifier inputs (a shape menu, radius, second radius, length, extents). Wire display, drawn in front, never rendered, in its own collection, tagged so exporters skip it. Object scale multiplies the shape: sphere radius by the largest axis, capsule radius by X and Z and length by Y, box extents by X, Y and Z. The node group draws exactly what the solver uses. Only shape-value changes rebuild geometry, so colliders stay on the fast draw path.
...
13. **No Unreal export.** The add-on exposes a versioned serialise / deserialise API for its setup (groups, settings, curves, links, colliders). Game packaging belongs to whoever consumes it; for WaifuSim that is the VRoid Swap add-on, which wraps all character metadata into a PNG.
...
- [ ] `serialize.py`: versioned `to_dict` / `from_dict` for groups, settings, curves (control points and dense samples), links, collider sets.
- [ ] Hair and skirt group presets.
- [ ] README with credits (Kawaii Physics, MIT) and the parity rule.
...
- **Render and post-frame-change writes.** Blender can miss data written in a post-frame-change handler during a render; the cache is the planned answer, to be verified in phase 6.
